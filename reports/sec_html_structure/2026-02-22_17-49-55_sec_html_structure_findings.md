...
**Generated:** 2026-02-22 17:49:55  
**Files analyzed:** 3  
**Mode:** reference files (AAPL_10K_2021, ADI_10K_2025, ALL_10K_2025)  
...
Analyzed **3 EDGAR full-submission text files** from the corpus. Key observations:

- Each file is an **SGML container** embedding 88–262 separate documents (mean 155).
- File sizes range from **10.5 MB to 46.5 MB** (mean 23.6 MB). The main 10-K iXBRL body is 4.3 MB on average.
- All 3 files have `MetaLinks.json` (100%) and `FilingSummary.xml` (100%).
- **All 23 DEI iXBRL tags** were fully present in 3/3 files. Tags with <100% presence are noted in §4.
- The SGML header provides reliable metadata for all core fields except `ein` (present in 33% of files). Use `dei:EntityTaxIdentificationNumber` instead.
...
```
min=10.5  max=46.5  mean=23.6  median=13.7  p95=46.5 MB
```

### 1.2 Main 10-K iXBRL body size

```
min=2.1  max=8.6  mean=4.3  median=2.4  p95=8.6 MB
```

The main 10-K body is the only document passed to `sec-parser`. Its size relative to the container explains why `sec-parser` is unaware of the other 154 embedded documents per filing.
...
|------|-------------|---------------|-----------------|--------------|
| `XML` | 384 | 3/3 | 34.16 | 128.0 |
| `GRAPHIC` | 34 | 3/3 | 2.42 | 11.3 |
| `10-K` | 3 | 3/3 | 13.01 | 1.0 |
| `EX-101.SCH` | 3 | 3/3 | 0.40 | 1.0 |
| `EX-101.CAL` | 3 | 3/3 | 0.61 | 1.0 |
| `EX-101.DEF` | 3 | 3/3 | 1.99 | 1.0 |
| `EX-101.LAB` | 3 | 3/3 | 4.19 | 1.0 |
| `EX-101.PRE` | 3 | 3/3 | 3.16 | 1.0 |
| `JSON` | 3 | 3/3 | 4.89 | 1.0 |
| `ZIP` | 3 | 3/3 | 4.51 | 1.0 |
| `EX-31.1` | 2 | 2/3 | 0.02 | 1.0 |
| `EX-31.2` | 2 | 2/3 | 0.02 | 1.0 |
| `EX-32.1` | 2 | 2/3 | 0.01 | 1.0 |
| `EXCEL` | 2 | 2/3 | 0.73 | 1.0 |
| `EX-21` | 2 | 2/3 | 0.17 | 1.0 |
| `EX-23` | 2 | 2/3 | 0.01 | 1.0 |
| `EX-97` | 2 | 2/3 | 0.05 | 1.0 |
| `EX-4.1` | 1 | 1/3 | 0.12 | 1.0 |
| `EX-21.1` | 1 | 1/3 | 0.01 | 1.0 |
| `EX-23.1` | 1 | 1/3 | 0.01 | 1.0 |
| `EX-4.9` | 1 | 1/3 | 0.02 | 1.0 |
| `EX-10.48` | 1 | 1/3 | 0.03 | 1.0 |
| `EX-19` | 1 | 1/3 | 0.07 | 1.0 |
| `EX-32.2` | 1 | 1/3 | 0.00 | 1.0 |
| `EX-10.26` | 1 | 1/3 | 0.01 | 1.0 |
| `EX-31.I` | 1 | 1/3 | 0.02 | 1.0 |
| `EX-32` | 1 | 1/3 | 0.00 | 1.0 |
...
```
min=67.0  max=213.0  mean=124.0  median=92.0  p95=213.0 sheets
```
...
|-------|---------|----------|-------|
| `accession_number` | 3/3 | 100% | Unique filing ID (not yet extracted by pipeline) |
| `cik` | 3/3 | 100% | SEC CIK — zero-padded 10 digits |
| `company_name` | 3/3 | 100% | All-caps legal name |
| `document_count` | 3/3 | 100% | Total embedded docs; varies 83–367 in this sample |
| `ein` | 1/3 | 33% ⚠️ | **Unreliable** — only present in some filings; use DEI instead |
| `filed_as_of_date` | 3/3 | 100% | YYYYMMDD filing date |
| `fiscal_year` | 3/3 | 100% | Derived from `period_of_report[:4]` |
| `fiscal_year_end` | 3/3 | 100% | MMDD format (e.g. `0925` = Sep 25) |
| `form_type` | 3/3 | 100% | Under FILING VALUES block; always `10-K` |
| `period_of_report` | 3/3 | 100% | YYYYMMDD — primary source for `fiscal_year` |
| `sec_file_number` | 3/3 | 100% | Exchange registration number |
| `sic_code` | 3/3 | 100% | Parsed from `sic_full` |
| `sic_full` | 3/3 | 100% | Raw string: `NAME [code]` |
| `sic_name` | 3/3 | 100% | Parsed from `sic_full` |
| `state_of_incorporation` | 3/3 | 100% | Two-letter code |
| `submission_type` | 3/3 | 100% | Always `10-K` |
...
|-----|---------|----------|-------|
| `EntityCentralIndexKey` | 3/3 | 100% | Duplicates SGML CIK; useful cross-check |
| `TradingSymbol` | 3/3 | 100% | **Ticker** — only source; two format variants |
| `EntityRegistrantName` | 3/3 | 100% | Formatted name with punctuation (vs all-caps SGML) |
| `DocumentFiscalYearFocus` | 3/3 | 100% | Year as integer string |
| `DocumentFiscalPeriodFocus` | 3/3 | 100% | Always `FY` for 10-K |
| `DocumentType` | 3/3 | 100% | Always `10-K` |
| `DocumentPeriodEndDate` | 3/3 | 100% | Human-readable date (may contain HTML entities) |
| `EntityIncorporationStateCountryCode` | 3/3 | 100% | Full state name (vs two-letter SGML code) |
| `EntityTaxIdentificationNumber` | 3/3 | 100% | EIN with hyphen — **reliable; use over SGML `ein`** |
| `EntityAddressAddressLine1` | 3/3 | 100% | Street address |
| `EntityAddressCityOrTown` | 3/3 | 100% | City |
| `EntityAddressStateOrProvince` | 3/3 | 100% | State code |
| `EntityAddressPostalZipCode` | 3/3 | 100% | ZIP code |
| `CityAreaCode` | 3/3 | 100% | Phone area code |
| `LocalPhoneNumber` | 3/3 | 100% | Local phone number |
| `Security12bTitle` | 3/3 | 100% | Security description; absent for non-12b filers |
| `SecurityExchangeName` | 3/3 | 100% | Exchange; absent for non-12b filers |
| `EntityWellKnownSeasonedIssuer` | 3/3 | 100% | WKSI status: Yes/No |
| `EntityFilerCategory` | 3/3 | 100% | Large accelerated / accelerated / non-accelerated |
| `EntityPublicFloat` | 3/3 | 100% | Market cap at mid-year; formatting varies |
| `EntityCommonStockSharesOutstanding` | 3/3 | 100% | Share count at recent date |
| `AmendmentFlag` | 3/3 | 100% | True/False/false — case varies across filers |
| `IcfrAuditorAttestationFlag` | 3/3 | 100% | SOX 404(b); may be HTML entity (☑/☐) |
...
| SIC | Count | % of sample |
|-----|-------|-------------|
| 3571 — ELECTRONIC COMPUTERS | 1 | 33% |
| 3674 — SEMICONDUCTORS & RELATED DEVICES | 1 | 33% |
| 6331 — FIRE, MARINE & CASUALTY INSURANCE | 1 | 33% |
...
```
min=24.0  max=55.0  mean=37.7  median=34.0  p95=55.0 days
```
...
| Value | Count | % |
|-------|-------|---|
| Large accelerated filer | 3 | 100% |

**Exchange Listed On** (`SecurityExchangeName`):

| Value | Count | % |
|-------|-------|---|
| The Nasdaq Stock Market LLC | 1 | 33% |
| Nasdaq Global Select Market | 1 | 33% |
...
Each `.html` file is a flat concatenation of 88–262 embedded documents delimited by `<DOCUMENT>` tags. The `sec-parser` library only processes Document 1 (the iXBRL 10-K body). All other documents — including XBRL financials, exhibits, and MetaLinks.json — are invisible to it.
...
`ein` was present in only 1/3 (33%) SGML headers, but `dei:EntityTaxIdentificationNumber` was present in 3/3 (100%) DEI blocks. Always prefer the DEI source for EIN.

**F4 — All 23 DEI tags were present in 100% of the 3 analyzed files.**  
Coverage may drop when extending to the full 961-file corpus.

**F5 — SGML header fields with <100% presence:** `ein` (1/3).

**F6 — File size variance is large.**  
Mean=23.6 MB, stdev=19.9 MB (outlier threshold: mean+1σ = 43.5 MB). Files above threshold: `ALL_10K_2025` (46.5 MB). Large files drive slower parse times in `sec-parser`.

**F7 — R*.htm sheet count is the primary driver of file size.**  
Sheets range from 67 to 213 per filing (mean 124). More sheets = more XBRL financial disclosures. This grows over time as reporting standards require more granular segment breakouts.

**F8 — MetaLinks.json and FilingSummary.xml are present in every file.**  
Both were found in 3/3 and 3/3 files respectively. MetaLinks.json is the authoritative XBRL element dictionary (FASB definitions, calculation trees, presentation hierarchy). FilingSummary.xml maps every R*.htm sheet to its human-readable name and `MenuCategory` (Statements/Notes/Details/etc.).
...
|----------------|---|----------|----------|-----------|
| Large accelerated filer | 3 | 24 | 55 | 38 |
